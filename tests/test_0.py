...
@pytest.mark.parametrize(
    "unit_type,valid_quantity,invalid_quantity",
    [
...
def test_check_method_rejects_non_quantities(invalid_input):
    ...
    with pytest.raises(AssertionError, match="Expected a pint Quantity"):
...
@pytest.mark.parametrize(
    "invalid_input",
    [None, 42, "10 joules", [100, ureg.joule], {"value": 100, "unit": "joule"}],
)
...
def test_physics_calculations_integration():
    ...
    assert numpy.isclose(
        work.to(ureg.kilowatt_hour).magnitude, 0.06
    )  # 60W * 1h = 0.06 kWh
